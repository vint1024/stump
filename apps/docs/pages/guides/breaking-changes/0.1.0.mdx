...
/stump-new/      # Your new Stump instance
	├── docker-compose.yml
	└── config/ # Empty, first start will create what you need
```
...
- If your existing setup uses `latest` alias for the Docker image, pin it to `0.0.12` instead to avoid accidental upgrades
- Make sure to fully stop your existing Stump instance before starting the new one (even though they are isolated)
...
- An entire mobile application:
  ...
  - Offline reading
- OIDC authentication support ([#246](https://github.com/stumpapp/stump/issues/246))
